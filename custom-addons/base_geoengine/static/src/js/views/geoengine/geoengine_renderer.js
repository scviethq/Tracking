...
//                    });
                    var projection = new ol.proj.get(this.mapOptions.geoengine_layers.projection);
                    var format = new ol.format.GeoJSON({
                        featureProjection: projection,
                        defaultDataProjection: 'EPSG:' + item.fields[cfg.geo_field_id[1]].srid,
                    });
                    var feature = new ol.Feature({
        ...
        _styleVectorLayerDefault: function (cfg, data) {
            ...
            if (cfg.symbols) {
                // TODO Need to implement field value check.
                styles = [];
                cfg.symbols.forEach(function(symbol) {
            ...
            return {
                style: function (feature) {
                    ...
                    var style_feature = _.find(styles, function(style) {
                        if (style && style.symbol && style.symbol.fieldname && style.symbol.value) {
                            return feature.values_.attributes[style.symbol.fieldname] == style.symbol.value;
                        } else {
                            return false;
                        }
                    ...
                    if (!style_feature) {
                        style_feature = styles[0];
                    }
                    style_feature.text_.text_ = label_text;
